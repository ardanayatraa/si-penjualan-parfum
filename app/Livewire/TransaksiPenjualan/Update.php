    ...
    public function update()
{
    ...
    DB::transaction(function () {
        ...
        // Hitung ulang total
        $harga_jual_total = $this->harga_jual * $this->jumlah_penjualan;
        $this->recalculate(); // supaya total_harga update
        ...
        $oldJurnals = JurnalUmum::whereDate('tanggal', $t->tanggal_transaksi)
            ->where('keterangan', 'like', "%Penjualan {$bNew->nama_barang}%")
            ->get();
        foreach ($oldJurnals as $jurnal) {
            $jurnal->delete();
        }

        // Buat ulang jurnal baru
        // Akun penjualan: 401 (kredit), Akun kas/piutang: 101 (debit)
        JurnalUmum::create([
            'id_akun'    => 101,
